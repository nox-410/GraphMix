...
Client::Client(int target, std::shared_ptr<ps::Customer> customer) {
  customer_= customer;
  // start zmq
...
void Client::connect(int target) {
  for (int i = 0; i < 2; i++) {
...
void Client::sendArriveMessage() {
  ...
  Message rmsg;
  RecvMsg(&rmsg);
  if (rmsg.meta.control.cmd != Control::ARRIVE) {
...
void Client::Receiving() {
  while (true) {
    ...
    customer_->Accept(msg);
  }
}

int Client::SendMsg(Message& msg, bool send_to_self) {
  // send meta
...
int Client::RecvMsg(Message* msg) {
  msg->data.clear();
  size_t recv_bytes = 0;
  for (int i = 0; ; ++i) {
