...
void Van::ProcessDataMsg(Message* msg) {
  ...
  auto* obj = Postoffice::Get()->GetCustomer(app_id, customer_id, 0);
  CHECK(obj) << "Customer not initilized " << customer_id << " ready at " << my_node_.role;
  obj->Accept(*msg);
}
...
void Van::Start(int customer_id) {
  ...
  if (init_stage == 0) {
    ...
    if (is_scheduler_) {
      ...
      int port = 0;
      if (Postoffice::Get()->is_server() && Environment::Get()->find("GRAPHMIX_SERVER_PORT"))
        port = atoi(Environment::Get()->find("GRAPHMIX_SERVER_PORT"));
      else
